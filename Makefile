...
space :=
space +=
MAIN_NAME = $(notdir $(subst $(space),_,$(CURDIR)))
MAIN_EXE = $(BUILD_ROOT)/$(MAIN_NAME).bin
MAIN_ELF = $(OBJ_DIR)/$(MAIN_NAME).elf
SRC_GIT_VERSION = $(call git_description,$(dir $(SRC)))
...
upload: all
	$(ESP_TOOL) $(UPLOAD_VERB) -cd nodemcu -cb $(UPLOAD_SPEED) -cp $(UPLOAD_PORT) -ca 0x00000 -cf $(MAIN_EXE)
